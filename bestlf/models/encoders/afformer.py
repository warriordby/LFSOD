...
class AFFormer(BaseModule):
    def __init__(
        ...
        self.conv_last = nn.ModuleList([nn.Conv2d(embed_dims[idx+1] if idx+1 != self.num_stages  else embed_dims[idx],\
                                         1, kernel_size=3, padding=1) for idx in range(self.num_stages)])
    ...
    def init_weights(self,pretrained):
        if isinstance(self.init_cfg, str) and pretrained==None:
    ...
    def forward(self, x):
        ...
        for idx in range(self.num_stages):
            ...
            outs.append(self.conv_last[idx](x))
            out.append(x)
            
        return out, outs[0], outs[1], outs[2], outs[3]
...
class afformer_tiny(AFFormer):
    ...
    def get_attributes(self, attribute_names):
        ...
        参数:
        attribute_names -- 一个包含属性名称的列表，这些属性将被返回。
        ...
        for attr_name in attribute_names:
            ...
            if hasattr(self, attr_name):
                # 将属性和它的值添加到字典中
                selected_attributes[attr_name] = getattr(self, attr_name)
