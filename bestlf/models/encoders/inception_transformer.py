...
class LowMixer(nn.Module):
    ...
    def forward(self, x):
        ...
        _,  _H, _W, _= xa.shape
        xa=  xa.view(B, -1, self.dim)# B H*W C
        B, N, C = xa.shape
        ...
        xa = xa.view(B, C, _H, _W)#.permute(0, 3, 1, 2)
        xa = self.uppool(xa)
        return xa
...
class InceptionTransformer(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dims=None, depths=None,
        ...
        self.conv_last = nn.ModuleList([nn.Conv2d(embed_dims[idx] , 1, kernel_size=3, padding=1) for idx in range(4)])
    ...
    def forward_features(self, x):
        ...
        out.append(self.norm0(x).permute(0, 3, 1, 2))

        x = x.permute(0, 3, 1, 2) #96    
        x = self.patch_embed2(x)
        B, H, W, C = x.shape
        x = x + self._get_pos_embed(self.pos_embed2, self.num_patches2, H, W) 
        x = self.blocks2(x)
        out.append(self.norm1(x).permute(0, 3, 1, 2))
        ...
        x = self.blocks3(x)
        out.append(self.norm2(x).permute(0, 3, 1, 2))
        ...
        x = self.blocks4(x)
        # x = x.flatten(1,2)
        out.append(self.norm3(x).permute(0, 3, 1, 2))
        outs=[]
        for idx in range(4):
            outs.append(self.conv_last[idx](out[idx]))
