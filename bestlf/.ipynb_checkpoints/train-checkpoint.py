...
from data import test_dataset, SalObjDataset
...
from mixup import mixup_images as mi
from mixup import mixup_images2 as mi2
...
def train(train_loader, model, optimizer, epoch, save_path):
    ...
    try:
        for i, (images, gts, focal) in enumerate(train_loader, start=1):            
            ...
            #focal_stack = mi(images,focal_stack)
            #images torch.Size([12, 3, 256, 256])
            #focal torch.Size([144, 3, 256, 256])
            #focal_stack 12个 torch.Size([12, 3, 256, 256])
            images,focal_stack = mi2(images,focal_stack)
            out,out1,out2,out3,out4= model(images)

            #loss = structure_loss(out, gts)+structure_loss(out3, gts3)+structure_loss(out4, gts4)
