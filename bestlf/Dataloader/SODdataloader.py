...
class test_dataset:
    ...
    def __init__(self, image_root, gt_root, focal_root, testsize):
        ...
        self.index = 0
    def load_data(self):
        image = self.rgb_loader(self.images[self.index])#(256, 256)
        image = self.transform(image).unsqueeze(0)  #torch.Size([1, 3, 256, 256])
        gt = self.binary_loader(self.gts[self.index]) #torch.Size([1, 3, 256, 256])
        focal = self.focal_loader(self.focals[self.index]) #(256, 256, 36)
        focal = np.array(focal, dtype=np.int32) #(256, 256, 36)
        if focal.shape[0] != 256:
            new_focal = []
            focal_num = focal.shape[2] // 3
            for i in range(focal_num):
